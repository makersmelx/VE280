...
{
    return !tree_isEmpty(tree_left(tree)) ? (fn(tree_elt(tree), tree_elt(tree_left(tree))) && tree_hasMonotonicPath_helper(tree_left(tree), fn)) : (!tree_isEmpty(tree_right(tree)) ? (fn(tree_elt(tree), tree_elt(tree_right(tree))) && tree_hasMonotonicPath_helper(tree_right(tree), fn)) : true);
}
